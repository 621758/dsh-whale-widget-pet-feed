...
export default {
  ...
  apply(ctx) {
    ...
    // —— 摸头切图：按绝对路径现读现发本地图片（前端「摸头图」留空时用下面的默认候选）——
    // 前端把路径放在 ?p= 里；只放行常见图片扩展名，避免这条接口退化成任意文件读取。
    // 每次请求都重新读盘，所以换图（覆盖 jjii.png）不用重启、也不用清缓存。
    const PET_IMAGE_MIME = {
      png: 'image/png', apng: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', jfif: 'image/jpeg',
      gif: 'image/gif', webp: 'image/webp', bmp: 'image/bmp', avif: 'image/avif',
    }
    // 默认摸头图：工作区里的 jjii.png → $DSH_HOME/jjii.png（都没有就用随包内置图）
    const PET_DEFAULT_CANDIDATES = [
      path.join(process.cwd(), 'jjii.png'),
      path.join(DSH_HOME, 'jjii.png'),
    ]
    // 随包发布的内置摸头图（assets/pet-default.png）：用户在设置里填的路径被删掉/挪走时兜底，
    // 保证「摸头切图」不会因为图片被误删就失效或报错。
    const PET_BUILTIN_CANDIDATES = [
      path.join(PACKAGE_ROOT, 'assets', 'pet-default.png'),
      path.join(PACKAGE_ROOT, 'assets', 'pet-default.jpg'),
    ]
    function petRequestedPath(req) {
      try {
        const q = new URL(String((req && req.url) || '/'), 'http://localhost').searchParams.get('p')
        return q ? String(q) : ''
      } catch (err) { return '' }
    }
    function resolvePetImage(requested) {
      // 顺序：用户填的路径 → 默认候选（工作区/jjii.png、$DSH_HOME/jjii.png）→ 内置图
      const candidates = (requested ? [requested] : PET_DEFAULT_CANDIDATES).concat(PET_BUILTIN_CANDIDATES)
      for (const raw of candidates) {
        try {
          const p = path.resolve(String(raw))
          const mime = PET_IMAGE_MIME[path.extname(p).slice(1).toLowerCase()]
          if (!mime) continue
          const st = fs.statSync(p)
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/pet-image.png',
      handler: (req, res) => {
        try {
          const found = resolvePetImage(petRequestedPath(req))
          if (!found) throw new Error('pet image not found')
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('pet image unavailable')
        }
      },
    }))

    // —— 投喂按钮图：米饭 / 可乐 / token ——
    // 优先用 DSH 文件夹里的同名图（米饭.png / 可乐.png / 硬币.png），缺失就回退到随包内置图，
    // 所以用户把这些图删掉/改名也不会让三个投喂按钮变成裂图。
    const FEED_IMAGE_ITEMS = {
      rice: { names: ['米饭.png', 'rice.png'], builtin: 'feed-rice.png' },
      cola: { names: ['可乐.png', 'cola.png'], builtin: 'feed-cola.png' },
      token: { names: ['硬币.png', 'token.png', 'coin.png'], builtin: 'feed-token.png' },
    }
    function feedRequestKind(req) {
      try {
        const k = new URL(String((req && req.url) || '/'), 'http://localhost').searchParams.get('kind')
        return k && FEED_IMAGE_ITEMS[k] ? String(k) : ''
      } catch (err) { return '' }
    }
    function resolveFeedImage(kind) {
      const item = FEED_IMAGE_ITEMS[kind]
      if (!item) return null
      const candidates = []
      for (const name of item.names) {
        // 工作区（DSH 会话目录）与 $DSH_HOME 下都找一遍，都没有就用随包内置图
        candidates.push(path.join(process.cwd(), name))
        candidates.push(path.join(DSH_HOME, name))
      }
      candidates.push(path.join(PACKAGE_ROOT, 'assets', item.builtin))
      for (const raw of candidates) {
        try {
          const p = path.resolve(String(raw))
          const mime = PET_IMAGE_MIME[path.extname(p).slice(1).toLowerCase()]
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/feed-image.png',
      handler: (req, res) => {
        try {
          const found = resolveFeedImage(feedRequestKind(req))
          if (!found) throw new Error('feed image not found')
          const bytes = fs.readFileSync(found.path)
          res.writeHead(200, {
            'Content-Type': found.mime,
            'Cache-Control': 'no-store',
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('feed image unavailable')
        }
      },
    }))
