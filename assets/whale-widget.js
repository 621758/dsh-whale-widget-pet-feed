...
// ===== 摸头切图（鼠标在角色上来回滑动）=====
// 鼠标在角色上左右来回划，累计几次「有效方向反转」（PET_MIN_FLIPS）→ 临时切到「摸头图」；
// 停手约 0.7 秒 → 切回当前角色图。
// 防误触发（只挡"顺手划过/手抖"，不挡正常撸）：
//   ① 每一笔（同方向累计位移）要 ≥ PET_MIN_SEG_PX，纯手抖（±3px 级别）不算；
//   ② 竖直分量超过水平分量 PET_MAX_TILT 倍的位移（上下划、斜着拉对角线）作废这一笔；
//   ③ 反转要挤在 PET_WINDOW_MS 内，且两次反转间隔 ≥ PET_FLIP_GAP_MS。
// 觉得不好摸就调小这 5 个数字，觉得太容易被蹭到就调大。
// 摸头图默认「自动」：工作区里的 jjii.png → $DSH_HOME/jjii.png → 随包内置的同一张；
// 可在主菜单里改成任意本地图片路径，整个功能也能用主菜单的「摸头切图」开关关掉。
// 配置存 localStorage('dshw-pet')；图片本身由宿主 /dsh-whale/pet-image.png?p=<绝对路径> 现读现发。
var PET_DEFAULT_PATH = '' // 留空 = 自动：工作区里的 jjii.png → $DSH_HOME/jjii.png → 随包内置图
var PET_MIN_FLIPS = 3      // 累计几次「有效方向反转」才算摸头（越少越敏感）
var PET_WINDOW_MS = 1400   // 这些反转要在这么长时间内完成
var PET_MIN_SEG_PX = 12    // 一笔（同方向）至少要划出这么多像素才算数
var PET_MAX_TILT = 1.8     // 单次位移竖直分量 > 水平分量 * 该值 → 不算左右滑动
var PET_FLIP_GAP_MS = 50   // 两次反转之间的最小间隔，避免手抖被连记
var PET_IDLE_MS = 700      // 停手多久切回角色图
var PET_SOUND_GAP_MS = 220 // 一直摸的时候，两次摸头音效之间的最小间隔（越小越密集）
var PET_CLICK_MUTE_MS = 400 // 点按（点击）开始后，这段时间内不再补摸头音效，避免叠成两遍
var petCfg = { on: true, path: PET_DEFAULT_PATH, soundOn: true }
try {
  var petCfgRaw = localStorage.getItem('dshw-pet')
  if (petCfgRaw) {
    var petCfgSaved = JSON.parse(petCfgRaw)
    if (petCfgSaved && typeof petCfgSaved === 'object') {
      if (petCfgSaved.on === false) petCfg.on = false
      if (petCfgSaved.soundOn === false) petCfg.soundOn = false
      if (typeof petCfgSaved.path === 'string' && petCfgSaved.path.replace(/\s/g, '')) petCfg.path = petCfgSaved.path.replace(/^\s+|\s+$/g, '')
    }
  }
} catch (err) {}
var petActive = false      // 当前显示的是摸头图
var petIdleTimer = null
var petTracked = false
var petLastX = 0
var petLastY = 0
var petSegX = 0            // 当前这一笔（同方向）累计的水平位移
var petLastDir = 0
var petFlips = 0
var petWindowAt = 0
var petFlipAt = 0
var petProbe = null        // 预载好的摸头图（Image 对象）
var petImgState = 'idle'   // idle | loading | ok | error
var petPathEl = null       // 菜单里的摸头图路径输入框（读图状态写进它的 title，界面上不再多一个钩）
var petSoundEl = null      // 菜单里的「音效」复选框（菜单建好后才有）
function petSaveCfg() {
  try { localStorage.setItem('dshw-pet', JSON.stringify({ on: petCfg.on, path: petCfg.path, soundOn: petCfg.soundOn })) } catch (err) {}
}
function petImageUrl(p) { return '/dsh-whale/pet-image.png?p=' + encodeURIComponent(String(p || '')) }
function petSyncState() {
  if (!petPathEl) return
  try {
    var tip = '摸头图的本地绝对路径（png / jpg / gif / webp）；留空 = 自动（工作区里的 jjii.png → $DSH_HOME/jjii.png → 随包内置图）'
    if (!petCfg.on) tip = '摸头切图已关闭 · ' + tip
    else if (petImgState === 'ok') tip = '已就绪：' + petCfg.path
    else if (petImgState === 'loading') tip = '正在读取：' + petCfg.path + ' …'
    else if (petImgState === 'error') tip = '读不到这张图（检查路径与格式，支持 png/jpg/gif/webp）：' + petCfg.path
    petPathEl.title = tip
  } catch (err) {}
}
// 预载摸头图：加载好之后摸头才是瞬间切换（否则切过去是空白）
function petLoadImage() {
  petProbe = null
  petImgState = 'idle'
  if (!petCfg.on) { petStop(); petSyncState(); return }
  petImgState = 'loading'
  petSyncState()
  try {
    var probe = new Image()
    probe.onload = function () { petProbe = probe; petImgState = 'ok'; petSyncState() }
    probe.onerror = function () { petProbe = null; petImgState = 'error'; petStop(); petSyncState() }
    probe.src = petImageUrl(petCfg.path)
  } catch (err) { petImgState = 'error'; petSyncState() }
}
function petShow() {
  if (!petCfg.on || petActive) return
  if (!petProbe || !petProbe.complete || !petProbe.naturalWidth) return
  // 投喂图正显示时让位（谁后发生谁显示），避免两个"临时图"互相抢 img.src
  if (feedActive) {
    if (feedTimer) { clearTimeout(feedTimer); feedTimer = null }
    feedActive = false
  }
  petActive = true
  try { img.src = petProbe.src } catch (err) { petActive = false }
  // 互动统计：真的切到摸头图才算一次摸头（继续摸不会重复计数，petActive 期间 petShow 直接返回）
  if (petActive) { try { interactPetTick() } catch (err) {} }
}
function petStop() {
  if (petIdleTimer) { clearTimeout(petIdleTimer); petIdleTimer = null }
  if (!petActive) return
  petActive = false
  // 投喂图还在显示时不要抢回来（让投喂图自己停留到点）
  try { if (!feedActive) img.src = currentRole.url } catch (err) {}
}
function petKeepAlive() {
  if (petIdleTimer) clearTimeout(petIdleTimer)
  petIdleTimer = setTimeout(function () { petIdleTimer = null; petStop() }, PET_IDLE_MS)
}
function petForgetTrack() { petTracked = false; petLastDir = 0; petFlips = 0; petWindowAt = 0; petSegX = 0 }
// 角色图的矩形范围（摸头图与角色图同框，用来在切图之后继续接住滑动）
function petInImageBox(e) {
  try {
...
// 全域 pointermove 里调用；over = 指针当前是否压在角色上
function petOnMove(e, over) {
  if (!petCfg.on) { petForgetTrack(); petStop(); return }
  // 起始必须真的压在角色像素上；已经摸起来之后，只要还在图片框内就继续算
  // （摸头图和角色图的轮廓不一定重合，否则切图后滑动会被判成"离开"而立刻回弹）
  if (!over && !(petActive && petInImageBox(e))) { petForgetTrack(); return }
  if (petActive) petKeepAlive()
  var x = e.clientX
  var y = e.clientY
  if (!petTracked) { petTracked = true; petLastX = x; petLastY = y; petSegX = 0; return }
  var dx = x - petLastX
  var dy = y - petLastY
  petLastX = x
  petLastY = y
  ...
  // 竖直为主（上下划、斜着拉）不算左右摸头：这一笔作废，避免路过/拉对角线被误判
  if (dir === 0 || ady > adx * PET_MAX_TILT) { petLastDir = 0; petSegX = 0; return }
  if (dir === petLastDir) { petSegX += adx; return }
  // 换方向：**上一笔得真的划够长**才算一次有效反转（小抖动、微调都不算）
  var segOk = petLastDir !== 0 && petSegX >= PET_MIN_SEG_PX
  petLastDir = dir
  petSegX = adx
  if (!segOk) return
  var now = Date.now()
  if (now - petFlipAt < PET_FLIP_GAP_MS) return
  petFlipAt = now
  if (!petWindowAt || now - petWindowAt > PET_WINDOW_MS) { petWindowAt = now; petFlips = 1 }
  else petFlips++
  if (petFlips >= PET_MIN_FLIPS) {
    petFlips = 0
    petWindowAt = now
    var petWasActive = petActive
    petShow() // 刚开始摸才切图
    petKeepAlive()
    // 摸头音效：第一下立刻响（forced）
    if (petActive) petPlaySound(!petWasActive)
  } else if (petActive) {
    // 已经在摸：之后每数到一次方向反转都补一声，只受 PET_SOUND_GAP_MS 限流 → 摸得越久越密集
    petPlaySound(false)
  }
}
function petSetOn(on) {
  petCfg.on = !!on
  petSaveCfg()
  if (!petCfg.on) { petForgetTrack(); petStop() }
  petLoadImage()
}
function petSetPath(p) {
  var v = String(p == null ? '' : p).replace(/^\s+|\s+$/g, '')
  petCfg.path = v || PET_DEFAULT_PATH
  petSaveCfg()
  petForgetTrack()
  petStop()
  petLoadImage()
...
// —— 摸头音效 ——
// 沿用挂件自己的音效系统：播当前音效组的「按下」音（默认小黄鸭），音量走同一个音量条，
// 音效开关关掉（或音量 0、或该音效组的按下槽留空）时自然静音。不额外加素材，也不用新的设置项。
var petAudio = null
var petAudioUrl = ''
var petSoundAt = 0
var petMuteUntil = 0
function petSetSoundOn(on) {
  petCfg.soundOn = !!on
  petSaveCfg()
  if (!petCfg.soundOn) {
    try { if (petAudio) dshwvSoundStop(petAudio) } catch (err) {}
  } else {
    petRebuildAudio()
  }
  if (petSoundEl) petSoundEl.checked = petCfg.soundOn
}
// 音效组变化时重建播放器（同一 URL 的解码缓存会被复用，音效组没换则只同步音量）
function petRebuildAudio() {
  try {
    var wanted = audioGroupSlotEmpty(soundSet, 'press') ? '' : ('/dsh-whale/sound/press.mp3?set=' + soundSet)
    if (petAudio && petAudioUrl === wanted) { petAudio.volume = soundVol; return }
    petAudioUrl = wanted
    if (!wanted) { petAudio = null; return }
    petAudio = dshwvSound(wanted)
    petAudio.preload = 'auto'
    petAudio.volume = soundVol
    dshwvWarm([wanted])
  } catch (err) {}
}
// force=true 表示「刚开始摸」的那一下，忽略最小间隔
function petPlaySound(force) {
  if (!petCfg.soundOn || !soundOn) return
  var now = Date.now()
  // 点按刚发生：让点按音（按下+松开）自己演完，这期间不补摸头音，避免听起来像响了两遍
  if (now < petMuteUntil) return
  if (!force && now - petSoundAt < PET_SOUND_GAP_MS) return
  petSoundAt = now
  try {
    var wanted = audioGroupSlotEmpty(soundSet, 'press') ? '' : ('/dsh-whale/sound/press.mp3?set=' + soundSet)
    if (!petAudio || petAudioUrl !== wanted) petRebuildAudio()
    if (!petAudio) return
    petAudio.volume = soundVol
...
// 点按优先：正在响的摸头音效立刻掐掉，并在 PET_CLICK_MUTE_MS 内不再补音。
// 于是「摸着摸着点一下」只会听到点按音（按下+松开）这一套，不会和摸头音叠成两遍。
function petSilence() {
  var now = Date.now()
  petSoundAt = now
  petMuteUntil = now + PET_CLICK_MUTE_MS
  try { if (petAudio) dshwvSoundStop(petAudio) } catch (err) {}
}

// ===== 投喂：角色左侧三个 Emoji 按钮（米饭 / 可乐 / token）=====
// 按钮本身就用 Emoji（🍚 / 🥤 / 🪙），不加载图片。
// 按下按钮 = 把角色图**切换成对应的食物图**（米饭.png / 可乐.png / 硬币.png），
// 停留 FEED_SHOW_MS 后切回当前角色图；同时：播当前音效组的「按下」音 + Emoji 飞进角色 + 角色抖一下 + 一句台词。
// 投喂图由宿主路由 /dsh-whale/feed-image.png?kind= 下发：
//   它先在「工作区」和 $DSH_HOME 下找同名图（米饭.png / 可乐.png / 硬币.png），
//   找不到就用随包内置的 assets/feed-*.png —— 所以删图/改名都不会切出裂图。
var FEED_SHOW_MS = 2200 // 按下投喂后，角色图停留在食物图上的时长（毫秒）
var FEED_HIDE_MS = 4200 // 三个投喂按钮：无操作多久自动收起（毫秒）
// 投喂按钮总开关（主菜单「投喂按钮」）：关掉后点角色也不再出现
var FEED_KEY = 'dshw-feed'
var feedCfg = { on: true }
try {
  var feedCfgRaw = localStorage.getItem(FEED_KEY)
  if (feedCfgRaw) {
    var feedCfgSaved = JSON.parse(feedCfgRaw)
    if (feedCfgSaved && typeof feedCfgSaved === 'object' && feedCfgSaved.on === false) feedCfg.on = false
  }
} catch (err) {}
function feedSaveCfg() {
  try { localStorage.setItem(FEED_KEY, JSON.stringify({ on: feedCfg.on })) } catch (err) {}
}
var FEED_ITEMS = [
  { kind: 'rice', label: '米饭', emoji: '🍚', lines: ['谢谢老大的米饭～', '好吃！再来一口～', '唔…好香～'] },
  { kind: 'cola', label: '可乐', emoji: '🥤', lines: ['咕嘟咕嘟…爽！', '嗝～好喝！', '冰的更好喝哦～'] },
  { kind: 'token', label: 'token', emoji: '🪙', lines: ['token 收到，老大最好了～', '嘿嘿，这个我喜欢～', '又有 token 啦！'] },
]
var feedBar = null
var feedSquishTimer = null
var feedOneShots = {}
var feedProbes = {}        // kind -> 预载好的食物图（Image），按下时才能瞬间切换
var feedActive = false     // 角色图当前显示的是食物图
var feedTimer = null
var feedBarOpen = false    // 投喂按钮是否展开
var feedHideTimer = null   // 自动收起计时器
function feedItemOf(kind) {
  for (var i = 0; i < FEED_ITEMS.length; i++) { if (FEED_ITEMS[i].kind === kind) return FEED_ITEMS[i] }
  return null
}
function feedImageUrl(kind) { return '/dsh-whale/feed-image.png?kind=' + encodeURIComponent(kind) }
// 投喂图统一走宿主路由（同名图 → 随包内置图 的回退在宿主里做）
function feedImageCandidates(item) {
  return [feedImageUrl(item.kind)]
}
// 预载三张食物图：按下按钮时要瞬间切换，不能等网络（没加载好就不切，免得切出空白）
// 候选地址按顺序试，第一个能加载的留下
function feedWarmImages() {
  for (var i = 0; i < FEED_ITEMS.length; i++) {
    (function (item) {
      try {
        var urls = feedImageCandidates(item)
        var tryAt = function (idx) {
          if (idx >= urls.length) return
          var probe = new Image()
          probe.onload = function () { feedProbes[item.kind] = probe }
          probe.onerror = function () { tryAt(idx + 1) }
          probe.src = urls[idx]
        }
        tryAt(0)
      } catch (err) {}
...
// 把角色图切到食物图，FEED_SHOW_MS 后切回当前角色图
function feedShowImage(kind) {
  try {
    var probe = feedProbes[kind]
    if (!probe || !probe.complete || !probe.naturalWidth) return false
    if (petActive) petStop() // 摸头图先让位（这次是投喂）
    feedActive = true
    img.src = probe.src
...
function feedStopImage() {
  if (feedTimer) { clearTimeout(feedTimer); feedTimer = null }
  if (!feedActive) return
  feedActive = false
  try { if (!petActive) img.src = currentRole.url } catch (err) {}
}
// 一次性音效：不碰点按的「按下/松开」状态机，但跟音效开关、音量联动
function feedPlayOneShot(url) {
  if (!soundOn || !url) return
  try {
    var el = feedOneShots[url]
    if (!el) {
      el = dshwvSound(url)
      el.preload = 'auto'
      feedOneShots[url] = el
...
function feedPlaySound() {
  try {
    // 该音效组的「按下」槽留空 → 静音（与点按口径一致）
    if (audioGroupSlotEmpty(soundSet, 'press')) return
    feedPlayOneShot('/dsh-whale/sound/press.mp3?set=' + soundSet)
  } catch (err) {}
}
function feedSquish() {
  try {
    body.style.transform = SQUISH
    if (feedSquishTimer) clearTimeout(feedSquishTimer)
    feedSquishTimer = setTimeout(function () {
      feedSquishTimer = null
      if (!pressing) body.style.transform = 'scaleY(1) scaleX(1)'
    }, 190)
  } catch (err) {}
}
// Emoji 从按钮飞进角色（fixed + body 直属：不受挂件翻转/缩放影响）
function feedFly(kind, btn) {
  try {
    var from = btn.getBoundingClientRect()
    var to = img.getBoundingClientRect()
    if (!from.width || !to.width) return
    var item = feedItemOf(kind)
    var size = Math.max(20, Math.round(from.width * 0.92))
    var fly = document.createElement('div')
    fly.className = 'dshwv-feedfly'
    fly.textContent = item ? item.emoji : ''
    fly.style.width = size + 'px'
    fly.style.height = size + 'px'
    fly.style.fontSize = Math.round(size * 0.86) + 'px'
    fly.style.left = Math.round(from.left + from.width / 2 - size / 2) + 'px'
    fly.style.top = Math.round(from.top + from.height / 2 - size / 2) + 'px'
    document.body.appendChild(fly)
    var dx = Math.round((to.left + to.width * 0.46) - (from.left + from.width / 2))
    var dy = Math.round((to.top + to.height * 0.34) - (from.top + from.height / 2))
    requestAnimationFrame(function () {
      fly.style.transform = 'translate(' + dx + 'px,' + dy + 'px) scale(.34) rotate(12deg)'
      fly.style.opacity = '0'
    })
    setTimeout(function () { try { if (fly.parentNode) fly.parentNode.removeChild(fly) } catch (err) {} }, 800)
  } catch (err) {}
}
function feedToast(kind) {
  try {
    var item = feedItemOf(kind)
    if (!item) return
    ...
    el.className = 'dshwv-feedtoast'
    el.textContent = line
    el.style.left = Math.round(r.left + r.width * 0.08) + 'px'
    el.style.top = Math.round(r.top + r.height * 0.02) + 'px'
    ...
    setTimeout(function () {
      ...
      setTimeout(function () { try { if (el.parentNode) el.parentNode.removeChild(el) } catch (err) {} }, 320)
    }, 1100)
  } catch (err) {}
}
function feedDo(kind, btn) {
  feedPlaySound()
  feedSquish()
  feedFly(kind, btn)
  feedToast(kind)
  if (!feedShowImage(kind)) {
    // 图还没预载好（例如刚打开页面就点）：临时补一次预载，下次按就正常
    try { feedWarmImages() } catch (err) {}
  }
}
feedBar = document.createElement('div')
feedBar.className = 'dshwv-feed'
// 三个投喂按钮：平时藏着，点一下角色才展开（feedBarShow）
function feedBarTouch() {
  if (feedHideTimer) { clearTimeout(feedHideTimer); feedHideTimer = null }
  if (!feedBarOpen) return
  feedHideTimer = setTimeout(function () {
    feedHideTimer = null
    feedBarHide()
  }, FEED_HIDE_MS)
}
function feedBarShow() {
  if (!feedBar || !feedCfg.on) return // 总开关关掉时，点角色也不出现
  feedBarOpen = true
  try { feedBar.classList.add('dshwv-feed-open') } catch (err) {}
  feedBarTouch()
}
// 主菜单「投喂按钮」开关
function setFeedOn(on) {
  feedCfg.on = !!on
  feedSaveCfg()
  if (!feedCfg.on) {
    try { feedBarHide() } catch (err) {}
  } else {
    try { feedBarShow() } catch (err) {} // 打开时顺手展开一下，给个反馈
  }
}
function feedBarHide() {
  if (feedHideTimer) { clearTimeout(feedHideTimer); feedHideTimer = null }
  feedBarOpen = false
  try { if (feedBar) feedBar.classList.remove('dshwv-feed-open') } catch (err) {}
}
// 鼠标停在按钮列上时先不收起，移开再重新计时（免得刚要按就没了）
feedBar.addEventListener('mouseenter', function () {
  if (feedHideTimer) { clearTimeout(feedHideTimer); feedHideTimer = null }
})
feedBar.addEventListener('mouseleave', function () { feedBarTouch() })
for (var feedIdx = 0; feedIdx < FEED_ITEMS.length; feedIdx++) {
  (function (item) {
    var btn = document.createElement('button')
    btn.type = 'button'
    btn.className = 'dshwv-feed-btn'
    btn.title = '投喂' + item.label
    var em = document.createElement('span')
    em.className = 'dshwv-feed-emoji'
    em.textContent = item.emoji
    btn.appendChild(em)
    btn.addEventListener('click', function (e) {
      e.stopPropagation()
      try { feedBarTouch() } catch (err) {}
      try { feedDo(item.kind, btn) } catch (err) {}
    })
    feedBar.appendChild(btn)
  })(FEED_ITEMS[feedIdx])
}
feedWarmImages()
...
var css = [
  ...
  // 摸头切图：本地图片路径输入框（读取状态写在它的 title 里，不在界面上多显示角标）
  '.dshwv-petpath{flex:1;min-width:0;border:1px solid rgba(32,49,112,.4);border-radius:6px;padding:2px 4px;font-size:11px;color:#203170;background:#fff;box-sizing:border-box}',
  ...
  // 互动统计：点击 / 摸头 各三行（今日 / 本月 / 总数）+ 说明
  '.dshwv-clickrow{display:flex;align-items:center;gap:8px;margin:0;padding:9px 2px;color:#203170;font-size:12px;border-bottom:1px dashed rgba(32,49,112,.22)}',
  '.dshwv-clickrow:last-child{border-bottom:0}',
  '.dshwv-clickgap{flex:0 0 auto;height:8px}',
  '.dshwv-clicklab{flex:1 1 auto;min-width:0;letter-spacing:.04em}',
  '.dshwv-clickval{flex:0 0 auto;font-size:16px;font-weight:700;letter-spacing:.02em}',
  '.dshwv-clickhint{margin-top:8px;color:#5b6f9e;font-size:11px;line-height:1.45;white-space:normal}',
  // 投喂：贴在角色图左侧那一列（图占右下 59.45%；末尾的 +12% 就是离图的间距，改大 = 离得更远）
  // 平时隐藏，点一下角色才出现；无操作 FEED_HIDE_MS 后自动收起
  '.dshwv-feed{position:absolute;right:calc(59.45% + 12%);bottom:8%;display:flex;flex-direction:column;gap:calc(var(--dshw-base) * 0.018);z-index:3;pointer-events:none;opacity:0;visibility:hidden;transform:translateX(6px);transition:opacity .18s ease,transform .18s ease,visibility .18s}',
  '.dshwv-feed.dshwv-feed-open{opacity:1;visibility:visible;transform:none}',
  '.dshwv-feed-btn{display:flex;align-items:center;justify-content:center;width:calc(var(--dshw-base) * 0.1);height:calc(var(--dshw-base) * 0.1);padding:0;box-sizing:border-box;border:1px solid rgba(32,49,112,.28);border-radius:50%;background:rgba(255,255,255,.86);box-shadow:0 2px 6px rgba(0,0,0,.16);cursor:pointer;pointer-events:none;transition:transform .12s ease,background .12s ease}',
  '.dshwv-feed.dshwv-feed-open .dshwv-feed-btn{pointer-events:auto}',
  '.dshwv-feed-btn:hover{transform:scale(1.09);background:#fff}',
  '.dshwv-feed-btn:active{transform:scale(.92)}',
  '.dshwv-feed-emoji{font-size:calc(var(--dshw-base) * 0.055);line-height:1;pointer-events:none;user-select:none;-webkit-user-select:none}',
  // 挂件翻转到左侧时整块会被镜像，Emoji 再翻回来，免得看着是反的
  '.dshwv-root.dshwv-left .dshwv-feed-emoji{transform:scaleX(-1)}',
  '.dshwv-feedfly{position:fixed;z-index:10002;pointer-events:none;display:flex;align-items:center;justify-content:center;line-height:1;filter:drop-shadow(0 2px 4px rgba(0,0,0,.2));transition:transform .72s cubic-bezier(.35,.75,.4,1),opacity .72s ease}',
  '.dshwv-feedtoast{position:fixed;z-index:10002;pointer-events:none;background:rgba(32,49,112,.92);color:#fff;font-size:12px;line-height:1.2;padding:5px 9px;border-radius:9px;white-space:nowrap;box-shadow:0 3px 10px rgba(0,0,0,.22);opacity:0;transition:opacity .22s ease,transform .4s ease;color-scheme:light}',
  // 预警与预算设置区
...
// —— 摸头切图：鼠标在角色上来回滑动 → 临时切到摸头图（默认 DSH 文件夹下的 jjii.png） ——
var petToggle = document.createElement('input')
petToggle.type = 'checkbox'
petToggle.className = 'dshwv-check'
petToggle.checked = petCfg.on
petToggle.title = '开启后：鼠标在角色上来回滑动（摸头）会切到摸头图，停手约 0.7 秒自动切回当前角色'
petToggle.addEventListener('change', function () { petSetOn(petToggle.checked) })
var rowPet = menuRow()
rowPet.appendChild(menuLabel('摸头切图'))
rowPet.appendChild(petToggle)
menuBox.appendChild(rowPet)
// —— 摸头音效：摸头时播当前音效组的「按下」音（默认开，跟着上面的音效开关/音量走） ——
petSoundEl = document.createElement('input')
petSoundEl.type = 'checkbox'
petSoundEl.className = 'dshwv-check'
petSoundEl.checked = petCfg.soundOn
petSoundEl.title = '摸头切图时播放声音（沿用当前音效组的「按下」音与音量）；音效关掉或音量为 0 时同样静音'
petSoundEl.addEventListener('change', function () { petSetSoundOn(petSoundEl.checked) })
var rowPetSound = menuRow()
rowPetSound.appendChild(menuLabel('摸头音效'))
rowPetSound.appendChild(petSoundEl)
menuBox.appendChild(rowPetSound)
var petPathInput = document.createElement('input')
petPathInput.type = 'text'
petPathInput.className = 'dshwv-petpath'
petPathInput.value = petCfg.path
petPathInput.spellcheck = false
petPathInput.title = '摸头图的本地绝对路径（png / jpg / gif / webp）；留空 = 自动：工作区里的 jjii.png → $DSH_HOME/jjii.png → 随包内置图'
petPathEl = petPathInput
function petCommitPathInput() {
  var v = String(petPathInput.value || '').replace(/^\s+|\s+$/g, '')
  if (v === petCfg.path) { petPathInput.value = petCfg.path; return }
  petSetPath(v)
  petPathInput.value = petCfg.path
  petToggle.checked = petCfg.on
}
petPathInput.addEventListener('keydown', function (e) {
  e.stopPropagation()
  if (e.key === 'Enter' || e.keyCode === 13) {
    e.preventDefault()
    petCommitPathInput()
    try { petPathInput.blur() } catch (err) {}
  }
})
petPathInput.addEventListener('change', petCommitPathInput)
petPathInput.addEventListener('blur', petCommitPathInput)
var petResetBtn = document.createElement('button')
petResetBtn.type = 'button'
petResetBtn.className = 'dshwv-roleimport'
petResetBtn.textContent = '默认'
petResetBtn.title = '恢复默认（自动）：工作区里的 jjii.png → $DSH_HOME/jjii.png → 随包内置图'
petResetBtn.addEventListener('click', function (e) {
  e.stopPropagation()
  petSetPath(PET_DEFAULT_PATH)
  petPathInput.value = petCfg.path
})
var rowPetPath = menuRow()
rowPetPath.appendChild(menuLabel('摸头图'))
rowPetPath.appendChild(petPathInput)
rowPetPath.appendChild(petResetBtn)
menuBox.appendChild(rowPetPath)
// —— 投喂按钮：三个 Emoji 按钮是否随「点角色」出现（无操作自动收起） ——
var feedToggle = document.createElement('input')
feedToggle.type = 'checkbox'
feedToggle.className = 'dshwv-check'
feedToggle.checked = feedCfg.on
feedToggle.title = '开启后：点一下角色会展开 🍚 / 🥤 / 🪙 三个投喂按钮（无操作约 4 秒自动收起）；关掉则完全不出现'
feedToggle.addEventListener('change', function () { setFeedOn(feedToggle.checked) })
var rowFeed = menuRow()
rowFeed.appendChild(menuLabel('投喂按钮'))
rowFeed.appendChild(feedToggle)
menuBox.appendChild(rowFeed)
petSyncState()
petLoadImage()
...
// ===== 互动统计：点角色 / 摸头 次数（今日 / 本月 / 总数 + 清空）=====
// 数据存 localStorage('dshw-click')（沿用原 key，之前的点击计数不会丢）：
//   { v:2, clicks:{ total:N, days:{ 'YYYY-MM-DD': n } }, pets:{ total:N, days:{...} } }
// 旧版 v1（{ total, days }）读回时当作「点击」迁移。
// 口径：点击 = 鼠标点在角色上那一下；摸头 = 每次真的触发摸头图切换算一次
//      （拖动角色、长按唤菜单、以及没真的切图都不算）。
// 界面复用「小鲸鱼记账」的二级视图容器 usagePanel（靠 subViewKind 区分渲染谁）。
var INTERACT_KEY = 'dshw-click'
var interactData = null
var interactMainEl = null
var interactValEls = null // 六个数字节点，便于原地刷新
function interactDayStr(d) {
...
function interactTodayStr() { return interactDayStr(new Date()) }
function interactNewBucket() { return { total: 0, days: {} } }
function interactReadBucket(raw) {
  var bucket = interactNewBucket()
  if (!raw || typeof raw !== 'object') return bucket
  bucket.total = Math.max(0, Math.floor(Number(raw.total) || 0))
  if (raw.days && typeof raw.days === 'object') {
    for (var k in raw.days) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(k)) continue
      var n = Math.floor(Number(raw.days[k]) || 0)
      if (n > 0) bucket.days[k] = n
    }
  }
  return bucket
}
function interactLoad() {
  if (interactData) return interactData
  interactData = { v: 2, clicks: interactNewBucket(), pets: interactNewBucket() }
  try {
    var raw = localStorage.getItem(INTERACT_KEY)
    if (raw) {
      var parsed = JSON.parse(raw)
      if (parsed && typeof parsed === 'object') {
        if (parsed.clicks || parsed.pets) {
          interactData.clicks = interactReadBucket(parsed.clicks)
          interactData.pets = interactReadBucket(parsed.pets)
        } else if (parsed.total !== undefined || parsed.days) {
          interactData.clicks = interactReadBucket(parsed) // 旧版：只有点击
        }
      }
    }
  } catch (err) {}
...
function interactPruneBucket(bucket, limitStr) {
  var days = {}
  for (var k in bucket.days) {
    if (k >= limitStr && bucket.days[k] > 0) days[k] = bucket.days[k]
  }
  bucket.days = days
  return bucket
}
function interactSave() {
  try {
    ...
    // 逐日明细只留最近 400 天；总数单独记在 total，不受裁剪影响
    var limit = new Date()
    limit.setDate(limit.getDate() - 400)
    var limitStr = interactDayStr(limit)
    localStorage.setItem(INTERACT_KEY, JSON.stringify({
      v: 2,
      clicks: interactPruneBucket(interactData.clicks, limitStr),
      pets: interactPruneBucket(interactData.pets, limitStr),
    }))
  } catch (err) {}
}
function interactBucketStats(bucket) {
  var todayStr = interactTodayStr()
  var monthPrefix = todayStr.slice(0, 7)
  var month = 0
  for (var k in bucket.days) { if (k.indexOf(monthPrefix) === 0) month += bucket.days[k] }
  return { today: bucket.days[todayStr] || 0, month: month, total: bucket.total }
}
function interactStats() {
  var data = interactLoad()
  return { clicks: interactBucketStats(data.clicks), pets: interactBucketStats(data.pets) }
}
// 记一次互动（kind: 'clicks' | 'pets'）；localStorage 很小，直接落盘，不怕突然关页面
function interactTick(kind) {
  var data = interactLoad()
  var bucket = kind === 'pets' ? data.pets : data.clicks
  bucket.total++
  var t = interactTodayStr()
  bucket.days[t] = (bucket.days[t] || 0) + 1
  interactSave()
  interactRefreshPanel()
}
function interactClickTick() { interactTick('clicks') }
function interactPetTick() { interactTick('pets') }
function interactClearAll() {
  interactData = { v: 2, clicks: interactNewBucket(), pets: interactNewBucket() }
  interactSave()
  interactRefreshPanel()
}
function interactRefreshPanel() {
  if (!interactValEls) return
  try {
    var s = interactStats()
    interactValEls.clickToday.textContent = String(s.clicks.today)
    interactValEls.clickMonth.textContent = String(s.clicks.month)
    interactValEls.clickTotal.textContent = String(s.clicks.total)
    interactValEls.petToday.textContent = String(s.pets.today)
    interactValEls.petMonth.textContent = String(s.pets.month)
    interactValEls.petTotal.textContent = String(s.pets.total)
  } catch (err) {}
}
function interactStatRow(label, value) {
  var row = document.createElement('div')
  row.className = 'dshwv-clickrow'
  var l = document.createElement('span')
  l.className = 'dshwv-clicklab'
  l.textContent = label
  var v = document.createElement('span')
  v.className = 'dshwv-clickval'
  v.textContent = value
  row.appendChild(l)
  row.appendChild(v)
  return { row: row, val: v }
}
// 二级界面内容：与「小鲸鱼记账」共用 usagePanel，所以进入本视图时把它重建
function buildInteractPanel() {
  usageMainEl = null // 内容已被本视图替换 → 下次回到记账视图时重建它的骨架
  usagePanel.innerHTML = ''
  var title = document.createElement('div')
  title.className = 'dshwv-usage-subtitle'
  title.textContent = '- = 互动统计 = -'
  usagePanel.appendChild(title)
  interactMainEl = document.createElement('div')
  interactMainEl.className = 'dshwv-usagebody'
  // 内容比记账视图短得多：撑满标题以下的空间并整体居中，免得菜单高度下留一大片空白
  interactMainEl.style.height = 'calc(100% - 26px)'
  interactMainEl.style.minHeight = '240px'
  interactMainEl.style.justifyContent = 'center'
  usagePanel.appendChild(interactMainEl)
  var s = interactStats()
  var c1 = interactStatRow('今日点击', String(s.clicks.today))
  var c2 = interactStatRow('本月点击', String(s.clicks.month))
  var c3 = interactStatRow('点击总数', String(s.clicks.total))
  var p1 = interactStatRow('今日摸头', String(s.pets.today))
  var p2 = interactStatRow('本月摸头', String(s.pets.month))
  var p3 = interactStatRow('摸头总数', String(s.pets.total))
  interactMainEl.appendChild(c1.row)
  interactMainEl.appendChild(c2.row)
  interactMainEl.appendChild(c3.row)
  var gap = document.createElement('div')
  gap.className = 'dshwv-clickgap'
  interactMainEl.appendChild(gap)
  interactMainEl.appendChild(p1.row)
  interactMainEl.appendChild(p2.row)
  interactMainEl.appendChild(p3.row)
  interactValEls = {
    clickToday: c1.val, clickMonth: c2.val, clickTotal: c3.val,
    petToday: p1.val, petMonth: p2.val, petTotal: p3.val,
  }
  var row = menuRow()
  var clearBtn = document.createElement('button')
  clearBtn.type = 'button'
  clearBtn.className = 'dshwv-usage-more'
  clearBtn.style.marginTop = '8px'
  clearBtn.textContent = '清空统计'
  clearBtn.title = '把点击与摸头的今日 / 本月 / 总数一起清零（会先确认）'
  clearBtn.addEventListener('click', function (e) {
    e.stopPropagation()
    showConfirm('确定清空互动统计吗？\n点击与摸头的今日、本月、总数都会归零，且不可恢复。', function () {
      try { interactClearAll() } catch (err) {}
    })
  })
  row.appendChild(clearBtn)
  interactMainEl.appendChild(row)
  var hint = document.createElement('div')
  hint.className = 'dshwv-clickhint'
  hint.textContent = '点击 = 点在角色上（拖动角色、长按唤菜单不计）；摸头 = 每次真的触发摸头图切换算一次。'
  interactMainEl.appendChild(hint)
}
// —— 主菜单入口行 ——
var rowInteract = menuRow()
var interactOpenBtn = document.createElement('button')
interactOpenBtn.type = 'button'
interactOpenBtn.className = 'dshwv-usage-more'
interactOpenBtn.style.flex = '1'
interactOpenBtn.style.margin = '0'
interactOpenBtn.textContent = '查看'
interactOpenBtn.title = '查看今日 / 本月 / 总次数（点击、摸头），可一键清空'
interactOpenBtn.addEventListener('click', function (e) {
  e.stopPropagation()
  try { showUsageSub('interact') } catch (err) {}
})
rowInteract.appendChild(menuLabel('互动统计'))
rowInteract.appendChild(interactOpenBtn)
menuBox.appendChild(rowInteract)
...
var subViewKind = '' // '' 主菜单 | 'usage' 小鲸鱼记账 | 'interact' 互动统计（两者共用 usagePanel 容器）
...
function showUsageSub(kind) {
  ...
  setUsageNavBtn(true)
  if (subViewKind === 'interact') buildInteractPanel()
  else renderUsagePanel()
...
function renderUsagePanel() {
  if (subViewKind === 'interact') return // 容器正被「互动统计」占用
...
function fillUsagePanel(d) {
  if (subViewKind === 'interact') return // 容器正被「互动统计」占用（可能有意料之外的延迟刷新）
...
root.appendChild(feedBar) // 投喂按钮：角色左侧一列（在 .dshwv-body 之外，不跟着挤压变形）
dshwBodyAppend(root)
...
function applySoundSet() {
  ...
  // 摸头音效跟着音效组走：换组时重建播放器并重新预热（音量变化由 petRebuildAudio 同步）
  try { petRebuildAudio() } catch (err) {}
}
function playPress() {
  if (!soundOn) return
...
function pressDown() {
  body.style.transform = SQUISH
  pressing = true
  // 点击优先：先把正在响的摸头音效掐掉（并压住之后一小段的补音），
  // 这样「摸着摸着点一下」只会有一次声音（点按的按下+松开），不会叠成两遍
  try { petSilence() } catch (err) {}
  playPress()
}
function pressUp() {
  body.style.transform = 'scaleY(1) scaleX(1)'
  pressing = false
  // 松手后同样压住一小段：让刚排期的松开音（可能还在响）单独演完，不叠上摸头音
  try { petSilence() } catch (err) {}
  if (pressEnded) {
...
function onDocPointerDown(e) {
  if (e.target && e.target.closest) {
    ...
    if (e.target.closest('.dshwv-feed') || e.target.closest('.dshwv-rolelist') || e.target.closest('.dshwv-audiolist') ||
...
function endDrag(e, clickAllowed, cancelled) {
  ...
  if (clickAllowed && !drag.moved) {
    ...
    try { interactClickTick() } catch (err) {}
    // 点一下角色 → 展开三个投喂按钮（无操作 FEED_HIDE_MS 后自动收起）
    try { feedBarShow() } catch (err) {}
    whaleClick()
    refresh(true)
    return
  }
